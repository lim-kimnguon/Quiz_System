...
public class AnswerController {
  ...
  @GetMapping
  List<Answer> List(){
    return answerService.ListAllAnswer();
  }

  @GetMapping("/{id}")
  Answer ListById(@PathVariable Integer id){
    return answerService.getAnswer(id);
  }

  @GetMapping("/question/{id}")
  List<Answer> ListByQuestionId(@PathVariable Integer id){
    return answerService.getAnswerByQuestionId(id);
  }

  @PostMapping
  void CreateAnswer(@RequestBody RequestAnswer requestAnswer){
    Answer answer = new Answer();

    answer.setName(requestAnswer.getName());
    answer.setScore(requestAnswer.getScore());
  ...
  ResponseEntity<?> UpdateQuestionLevel(@RequestBody RequestAnswer requestAnswer, @PathVariable Integer id){
      try{
        ...
        answerUpdate.setScore(requestAnswer.getScore());
